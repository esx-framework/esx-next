...
export class Player implements AugmentableComponent {
    ...
    constructor(private readonly src: number, overrodeIdent?: string, overrodeIdentKind?: Identifiers) {
        if (overrodeIdent && overrodeIdentKind) {
                    this.idents.set(overrodeIdentKind, overrodeIdent)
        } else {
    ...
    /**
     * Create a player without source, only with an identifier (most methods obviously won't work)
     * @param identifier 
     * @param type 
     */
    public static offline(identifier: string, type: Identifiers) {
        return new Player(0, identifier, type)
    }
